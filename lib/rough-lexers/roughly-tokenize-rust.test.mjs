import assert, { strictEqual as eq, deepStrictEqual as deep } from 'node:assert';
import { renderNotice } from '../notices/render-notice.mjs';
import { roughlyTokenizeRust } from './roughly-tokenize-rust.mjs';

/**
 * @typedef {import('./roughly-tokenize-rust.mjs').Token} Token
 */
 ...
 * @param {string[]} expectedNotices The expected rendered notices
 * @returns {Token[]} The tokens produced during roughly tokenization
 */
const notice = (source, ...expectedNotices) => {
    ...
    eq(
        ...
        notices.map((n) => renderNotice(...n)).join('\n  ')
    );
